...
export class GameService {
  ...
  calculateScores([...scores]) {
    ...
    const newScore = (score, index) => {
      return score + 32 * (index - expectedScore(index ? a : b, index ? b : a));
    };
  ...
  async saveGame(gameData: SaveGame.request) {
    ...
    try {
      const looserId =
        gameData.score1 < gameData.score2 ? gameData.player1 : gameData.player2;
      const winnerId =
        gameData.score1 > gameData.score2 ? gameData.player1 : gameData.player2;
      const winner = await this.userService.updateGamesWinPlayed(winnerId);
      const looser = await this.userService.updateGamesLostPlayed(looserId);
